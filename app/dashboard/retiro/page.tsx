...
const APPS_SCRIPT_HABITACIONES = 'https://script.google.com/macros/s/AKfycbwsWG7DYnWKVlJzoydoMCpcRu1_I14JzBKWhljkF0brYyUc14MRDrJr6nhnR5oobm4eVA/exec'
...
export default function RetiroDashboard() {
  ...
  const generarCuartosAleatorio = async () => {
    ...
    try {
      ...
      for (const servidor of servidoresShuffled) {
        ...
        if (!hab) break
        nuevasAsignaciones.push({ habitacion_id: hab.id, persona_id: servidor.id, tipo_persona: 'servidor', nombre: servidor.nombre, retiro_id: RETIRO_ID })
        cuentaHab[hab.id]++
      }
      ...
      if (nuevasAsignaciones.length > 0) {
        await supabase.from('asignaciones_habitacion').insert(nuevasAsignaciones)
      }
  ...
  const sincronizarMesasSheets = async () => {
    try {
      ...
      await fetch(APPS_SCRIPT_MESAS, { method: 'POST', body: JSON.stringify({ tipo: 'actualizar_hoja_mesas', mesas: Object.values(porMesa) }) })
        ...
        <div>
          ...
          {loadingCuartos ? (
            ...
            <>
              ...
              {personasSinCuarto.filter(p => p.tipo === 'caminante').length > 0 && (
                <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: '#991b1b', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    {personasSinCuarto.filter(p => p.tipo === 'caminante').length} sin cuarto asignado
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    {personasSinCuarto
                      .filter(p => p.tipo === 'caminante')
                      .slice(0, 8)
                      .map(p => (
                        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#7f1d1d' }}>
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 20, background: '#f0fdf4', color: '#166534', flexShrink: 0 }}>
                            CAM
                          </span>
                          {p.nombre}
                        </div>
                      ))}
                    {personasSinCuarto.filter(p => p.tipo === 'caminante').length > 8 && (
                      <span style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>
                        ...y {personasSinCuarto.filter(p => p.tipo === 'caminante').length - 8} más
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                {[{ label: 'Todos', val: 'todos' as const }, { label: 'Servidores', val: 'servidor' as const }, { label: 'Caminantes', val: 'caminante' as const }].map(f => (
                  <button key={f.val} onClick={() => setFiltroTipoCuartos(f.val)} style={{ padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 600, background: filtroTipoCuartos === f.val ? '#7c3aed' : '#f3f4f6', color: filtroTipoCuartos === f.val ? 'white' : '#6b7280' }}>{f.label}</button>
                ))}
              </div>
              ...
              {habitaciones.length === 0 ? (
                <div style={{ background: 'white', border: '1.5px solid #e8eaf0', borderRadius: 14, padding: '40px 24px', textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Ejecuta el SQL en Supabase para cargar las habitaciones.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  {bloquesCuartos.map(bloque => {
                    const habsDeBloque = habitacionesFiltradas.filter(h => h.bloque === bloque)
                    if (habsDeBloque.length === 0) return null
                    const piso = habsDeBloque[0].piso
                    const colorBloque = piso === 1 ? '#cfe2ff' : piso === 2 ? '#f0fdf4' : '#faf5ff'
                    const colorTextoBloque = piso === 1 ? '#1e40af' : piso === 2 ? '#166534' : '#6b21a8'
                    return (
                      <div key={bloque}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: colorBloque, color: colorTextoBloque }}>Piso {piso}</span>
                          <h3 style={{ fontSize: 13, fontWeight: 700, color: '#111827', margin: 0 }}>{bloque}</h3>
                          {piso === 1 && <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: '#cfe2ff', color: '#1e40af' }}>Solo servidores</span>}
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {habsDeBloque.map(hab => {
                            ...
                            const tieneServidores = asigs.some(a => a.tipo_persona === 'servidor')
                                ...
                                <button onClick={() => setHabExpandida(abierta ? null : hab.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                                  ...
                                  <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                                      ...
                                      {tieneServidores && <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 20, background: '#cfe2ff', color: '#1e40af' }}>SRV</span>}
                                      {tieneCaminantes && <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 20, background: '#f0fdf4', color: '#166534' }}>CAM</span>}
                                    </div>
